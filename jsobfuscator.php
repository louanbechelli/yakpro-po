...
class Obfuscator
{
    ...
    static function isValidDomain($d)
    {
        return (preg_match("/^([a-z\d](-*[a-z\d])*)(\.([a-z\d](-*[a-z\d])*))*$/i", $d) && preg_match("/^.{1,253}$/", $d) && preg_match("/^[^\.]{1,63}(\.[^\.]{1,63})*$/", $d));
    }
    static function html2Js($c)
    {
        return "document.write('" . addslashes(preg_replace(self::$search, self::$replace, $c) . " ") . "');";
    }
    static function cleanHtml($c)
    {
        return preg_replace('/<!--(.|\s)*?-->/', '', $c);
    }
    static function cleanJS($c)
    {
        return preg_replace(self::$search, self::$replace, preg_replace('/(?:(?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/)|(?:(?<!\:|\\\|\')\/\/.*))/', '', $c));
    }
}
